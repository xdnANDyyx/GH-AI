...
public static final String uuidFilename(MultipartFile file) {
    ...
    public static final String getPathFileName(String uploadDir, String fileName) throws IOException {
        ...
        String currentDir = "";
        if (uploadDir != null && uploadDir.length() > baseDir.length()) {
            // uploadDir 可能是 baseDir 的子目录（如 baseDir/avatar）
            currentDir = uploadDir.substring(baseDir.length());
        }
        // 统一为 / 分隔符，并移除前导斜杠，避免生成双斜杠路径
        ...
        if (StringUtils.isEmpty(currentDir)) {
            // 上传目录就是 profile 本身 -> /profile/2026/08/05/xxx.jpg（单斜杠）
