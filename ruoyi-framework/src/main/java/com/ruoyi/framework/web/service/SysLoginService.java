...
//        validateCaptcha(username, code, uuid);
        ...
        // 记录登录信息（移除管理员限制，允许普通用户通过 /login 登录）
        recordLoginInfo(loginUser.getUserId());
        // 按用户类型签发对应 token，避免客户用户访问 /customer/** 时被 system token 拒绝
        return UserType.CUSTOMER.getCode().equals(user.getUserType())
            ? customerTokenService.createToken(loginUser)
            : tokenService.createToken(loginUser);
    }

    public String loginBySms(String phone, String smsCode) {
        loginPreCheckPhone(phone);
        smsCodeService.verifyLoginCode(phone, smsCode);

        SysUser user = userService.selectUserByPhoneNumberAndUserType(phone, UserType.ADMIN.getCode());
    ...
    private void loginPreCheckPhone(String phone) {
