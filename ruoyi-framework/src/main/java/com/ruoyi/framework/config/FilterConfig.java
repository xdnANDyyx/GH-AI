...
import com.ruoyi.common.filter.ApiRequestLogFilter;
...
public class FilterConfig {
    ...
    @SuppressWarnings({ "rawtypes", "unchecked" })
    @Bean
    @ConditionalOnProperty(value = "api-log.enabled", havingValue = "true", matchIfMissing = false)
    public FilterRegistrationBean apiRequestLogFilterRegistration() {
        FilterRegistrationBean registration = new FilterRegistrationBean();
        registration.setDispatcherTypes(DispatcherType.REQUEST);
        registration.setFilter(new ApiRequestLogFilter());
        registration.addUrlPatterns("/*");
        registration.setName("apiRequestLogFilter");
